...
提供独立命令 **`undoproxy-gen`**，对任意带复杂嵌套 **Map / Slice / Pointer / struct** 的 Go 结构体定义，自动生成 Undo 写代理（语义见 [docs/guide/proxy-api.md](../../guide/proxy-api.md)）：
...
- 跨包嵌套 struct 字段（当前仅**同包**可达类型；见 §7）。
...
3. 扩展 `types.go` fixture（可选 `Hero`/`Skill` 嵌套 map，对齐嵌套 map 夹具）。
...
- [ ] `Player` 上 `Put*` / `Append*` / `Set*At` / `Remove*At` / `Truncate*` / `Get*ForWrite` 与早期手写行为一致（含 Undo/Rollback）
...
- [docs/guide/proxy-api.md](../../guide/proxy-api.md) — 生成代理用法
- [docs/superpowers/specs/2026-05-25-cow-undo-log-design.md](../specs/2026-05-25-cow-undo-log-design.md) — Undo Log
- `zz_generated.undo_proxy.go` — 生成结果参考
